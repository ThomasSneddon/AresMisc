...
#define INI_READ(item, addr) \
	...
	ColorStruct* ReadColor(ColorStruct* pBuffer, const char* pSection, const char* pKey, ColorStruct const& def)
		{ JMP_THIS(0x474C70); }

	ColorStruct ReadColor(const char* const pSection, const char* const pKey, ColorStruct const& def) {
		ColorStruct outBuffer;
		this->ReadColor(&outBuffer, pSection, pKey, def);
		return outBuffer;
	}
